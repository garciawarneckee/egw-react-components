...
export default class DragabbleButton extends Component {
  ...
  constructor(props) {
    ...
    this.state = {
      ...
      style: null,
      onStart: () => { throw new Error('onStart function is not defined') },
      onDrag: () => { throw new Error('onDrag function is not defined') },
      onStop: () => { throw new Error('onStop function is not defined') }
    }
  }

  setButtonContent(iconName, text) {
    const fontAwesomeIcon = (<FontAwesomeIcon className='icon' icon={iconName}/>); 
